      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for(int i = 0; i< sensor_fusion.size(); i++){
				//car is in my lane
				//sensor_fusion[1][6] gives the d value
				float d = sensor_fusion[i][6];
				if(d < (2+4*lane+2) && d > (2+4*lane-2)){
					...
					//since we are always appending to 50 ponits in next_x_vals and next_y_vals (means we are telling our car what to do in future),
					// we also want to have an idea of the cars in our line in that future time. That is why we are predicting the position of cars in our
					//lane 0.02t in the future. 
					...
					if((check_car_s > car_s ) && (check_car_s - car_s - 30)){
						...
						//if car in front of us in our lane is slow, slow down and try to shift the lane.
